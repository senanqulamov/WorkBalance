...
<div x-data="tallstackui_table({!! $entangle !!}, @js($ids()), @js($selectable))" @if ($persistent) x-ref="persist" @endif>
    ...
    <div class="{{ $personalize['wrapper'] }}">
        <div class="{{ $personalize['table.wrapper'] }}"
             style="position: relative; min-height: 400px; isolation: isolate;"
             @if ($livewire && $loading)
                 wire:loading.delay.class="overflow-hidden max-h-[400px]"
                 wire:target="{{ $target }}"
             @endif>
            @if ($livewire && $loading)
                <div wire:loading.delay="{{ $target }}"
                     class="hidden fx-table__loading"
                     wire:loading.delay.class.remove="hidden"
                     style="--fx-table-cols: {{ count($headers) + ($selectable ? 1 : 0) }};">
                    <div class="fx-table__skeleton" aria-hidden="true">
                        {{-- Header row --}}
                        @php
                            $cols = count($headers) + ($selectable ? 1 : 0);
                        @endphp
                        <div class="fx-table__skeleton-row" style="grid-template-columns: repeat({{ $cols }}, minmax(0, 1fr));">
                            @for ($c = 0; $c < $cols; $c++)
                                <div class="fx-table__skeleton-cell"></div>
                            @endfor
                        </div>
                        {{-- Data rows (10 visible rows) --}}
                        @for ($r = 0; $r < 10; $r++)
                            <div class="fx-table__skeleton-row" style="grid-template-columns: repeat({{ $cols }}, minmax(0, 1fr));">
                                @for ($c = 0; $c < $cols; $c++)
                                    <div class="fx-table__skeleton-cell"></div>
            ...
            <table class="{{ $personalize['table.base'] }}"
                   @if ($livewire && $loading)
                       wire:loading.delay.class="opacity-30 pointer-events-none blur-sm select-none"
                       wire:target="{{ $target }}"
                   @endif>
                @if (!$headerless)
