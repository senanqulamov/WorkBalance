    ...
    <x-card>
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h2 class="text-lg font-semibold text-[var(--color-text-high)]">
                        @lang('Edit Quote for :title', ['title' => $quote->request->title])
                    </h2>
                    <p class="text-sm text-[var(--color-text-muted)]">
                        @lang('Buyer: :name', ['name' => $quote->request->buyer?->name ?? __('Unknown')])
                    </p>
                </div>

                <div class="text-xs text-[var(--color-text-muted)] text-right">
                    <div>
                        @lang('RFQ #:id', ['id' => $quote->request_id])
                    </div>
                    <div>
                        @lang('Quote #:id', ['id' => $quote->id])
                    </div>
                </div>
            </div>
        </x-slot:header>
        ...
        <form wire:submit.prevent="save" class="space-y-6">
            ...
            <div class="space-y-4">
                ...
                @foreach($quote->request->items as $item)
